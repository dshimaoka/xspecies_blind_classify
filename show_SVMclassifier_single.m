function [thisAx] = show_SVMclassifier_single(classifier_cv, ...
     thisAx, p_value, p_th)
%
...
mweight = mean(classifier_cv.weight,2);

%[~, sigIdx] = sort(mweight, 'descend');

histogram(thisAx, mweight);
xlabel(thisAx, 'mean weight across cv partitions');
